...
-- A ligação guarda o UUID Browns sem restaurar a tabela de credenciais antiga.
-- A reconciliação do manifesto valida a integridade no cutover.
select is(
  (select count(*)::int from pg_constraint
   where conrelid = 'public.pelada_memberships'::regclass and contype = 'f'
     and pg_get_constraintdef(oid) like '%players%'),
  0,
  'legacy_player_id nao exige restaurar a tabela de credenciais do legado'
);
...
-- As contas mantêm os profiles automáticos: o consumo deve fundir o profile
-- vazio com o histórico Browns, que é o percurso real depois do primeiro login.
