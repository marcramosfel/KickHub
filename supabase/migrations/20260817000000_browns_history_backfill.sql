-- KickHub V2 — backfill do histórico real da Pelada Browns.
--
-- Esta migration não transporta dados entre projetos. O dump da Browns deve
-- primeiro ser restaurado nas tabelas legadas `public.players`, `matches`, ...
-- durante o write freeze descrito em `docs/foundation-gates.md`. Depois, esta
-- função projeta esse histórico nas entidades multi-tenant sem apagar nem
-- renomear a origem.
--
-- A função é idempotente e fica disponível apenas para `service_role`, para
-- permitir repetir a reconciliação em staging depois de uma nova restauração.
-- PINs, senha administrativa e tokens de dispositivo não são usados aqui.

create or replace function public.browns_profile_id(p_player_id uuid)
returns uuid
language sql immutable strict
set search_path = public, pg_temp
as $$
  select md5('kickhub:browns:profile:' || p_player_id::text)::uuid;
$$;

create or replace function public.browns_membership_id(p_player_id uuid)
returns uuid
language sql immutable strict
set search_path = public, pg_temp
as $$
  select md5('kickhub:browns:membership:' || p_player_id::text)::uuid;
$$;
...
-- --------------------------------------------------------- perfil do jogador
-- Um único read model traz a identidade global e a linha desportiva do próprio
-- utilizador em cada pelada. A consulta não recebe profile_id: deriva sempre de
-- auth.uid(), impedindo que alguém troque um parâmetro para ler um perfil
-- privado. O overall continua a ser calculado no domínio TypeScript, mas a RPC
-- inclui os títulos e a média dos guarda-redes necessários para que o número
-- seja exatamente o mesmo do ranking.
...
revoke all on function public.browns_profile_id(uuid) from public, anon, authenticated;
revoke all on function public.browns_membership_id(uuid) from public, anon, authenticated;
...
-- Se os dados já foram restaurados antes do deploy, o backfill acontece na
-- própria migration. Em instalações limpas é um no-op; depois de uma
-- restauração tardia, `service_role` pode chamar a mesma função novamente.
