-- KickHub V2 — o `@` do jogador passa a ser o nome dele.
--
-- O backfill derivava o `username` do id legado: `br-1e76fa75612141c2abda1113`.
-- Era um identificador garantidamente único e completamente ilegível, e é o que
-- aparece no perfil e na URL `/u/:username`. O `@` de quem joga tem de ser o
-- nome de quem joga.
--
-- `Marcos Felipe` → `marcos_felipe`, `Félix` → `felix`, `Alisson Vaz` →
-- `alisson_vaz`.
--
-- Os acentos são tratados por `translate` e não por `unaccent`: a extensão pode
-- não estar instalada no destino, e falhar o cutover por causa disso seria um
-- preço absurdo por uma tabela de caracteres que cabe em duas linhas.
--
-- A atribuição não pode acontecer dentro do `insert` dos perfis. Duas pessoas
-- com o mesmo nome produziriam o mesmo `@`, e uma instrução não vê as linhas que
-- ela própria está a escrever — as duas ficariam com `ana` e a chave única
-- rejeitava a segunda. Por isso corre a seguir, uma linha de cada vez, e é isso
-- que permite à segunda descobrir que `ana` já foi tomada e ficar com `ana_2`.
--
-- Perfis já reclamados não são tocados. Depois de alguém entrar na conta, o `@`
-- é o endereço dessa pessoa, e mudá-lo por baixo dela partia o link do perfil.

create or replace function public.browns_username_from_name(p_name text)
returns text
...
-- Só o backfill a usa, e ele corre como `security definer`. Não há motivo para
-- a pôr ao alcance do browser.
revoke all on function public.browns_username_from_name(text) from public, anon, authenticated;
grant execute on function public.browns_username_from_name(text) to service_role;
...
begin
  ...
  set display_name = excluded.display_name,
      -- Uma foto que ja subiu para o bucket fica onde esta. Sem isto, repetir o
      -- backfill depois de `data:migrate-media` devolvia toda a gente a
      -- `pending` e apagava o caminho do objeto que ja la estava.
      avatar_path = case
        when public.profiles.avatar_status = 'ready' then public.profiles.avatar_path
        else excluded.avatar_path
      end,
      avatar_status = case
        when public.profiles.avatar_status = 'ready' then 'ready'
        else excluded.avatar_status
      end
  -- Só enquanto o perfil for uma projeção do legado. Depois de alguém o
  -- reclamar, o nome é dessa pessoa: uma nova passagem do backfill não pode
  -- desfazer o que ela escreveu. Sem esta cláusula, repetir a importação com
  -- outro modo de identidade deixava toda a gente com o nome antigo, porque
  -- o `do nothing` nunca reconciliava.
  where public.profiles.auth_user_id is null;

  -- ------------------------------------------------------------- @ do jogador
  -- O `username` nasce do id legado porque, no momento do insert, duas pessoas
  -- com o mesmo nome produziriam o mesmo `@` e a instrução não vê as linhas que
  -- ela própria está a escrever. A conversão para o nome acontece aqui, uma
  -- linha de cada vez, que é o que permite a segunda "Ana" descobrir que a
  -- primeira já ficou com `ana`.
  --
  -- Só toca em perfis ainda não reclamados: depois de alguém entrar na conta, o
  -- `@` é o endereço dessa pessoa e mudá-lo por baixo dela partia o link do
  -- perfil dela.
  for v_perfil in
    select profile.id, profile.display_name
    from public.profiles profile
    join public.pelada_memberships member on member.profile_id = profile.id
    where member.pelada_id = v_browns
      and member.legacy_player_id is not null
      and profile.auth_user_id is null
    order by profile.created_at, profile.id
  loop
    v_base := public.browns_username_from_name(v_perfil.display_name);
    ...
    while exists (
      ...
      where outro.username = v_candidato and outro.id <> v_perfil.id
    ) loop
      v_sufixo := v_sufixo + 1;
      v_candidato := v_base || '_' || v_sufixo::text;
    end loop;

    update public.profiles set username = v_candidato
    where id = v_perfil.id and username is distinct from v_candidato;
  end loop;
  ...
  set profile_id = coalesce(public.pelada_memberships.profile_id, excluded.profile_id),
      -- A alcunha manda no nome que o ranking mostra. Reconcilia pela mesma
      -- regra do perfil: enquanto ninguém reclamou a identidade, ela vem do
      -- legado; a partir daí é do dono.
      nickname = case
        when exists (
          select 1 from public.profiles claimed
          where claimed.id = public.pelada_memberships.profile_id
            and claimed.auth_user_id is not null
        ) then public.pelada_memberships.nickname
  ...
  --
  -- A equipa vem de três sítios, por esta ordem, porque o legado guardou-a de
  -- maneiras diferentes conforme a época. Exigir `goalkeeper_match_stats.team`
  -- deixava de fora exactamente a linha que isto veio reparar: a única rodada
  -- em que o guarda-redes existe só aqui é também a única em que a coluna ficou
  -- nula, e o filtro saltava-a.
  insert into public.game_lineups (
    game_id, membership_id, pelada_id, team, is_goalkeeper,
    overall_at_draw, overall_estimated, created_at
  )
  ...
  left join public.match_stats stat
    on stat.match_id = keeper.match_id and stat.player_id = keeper.goalkeeper_id
  left join public.matches resultado on resultado.id = keeper.match_id
  cross join lateral (
    select case
      when keeper.team in ('A', 'B') then keeper.team
      when stat.team in ('A', 'B') then stat.team
      -- Sem equipa registada em lado nenhum, os golos sofridos dizem de que
      -- lado ele estava: quem guarda a baliza de A sofre os golos de B. Só
      -- vale quando um dos lados bate e o outro não — num empate os dois
      -- batem, e aí prefere-se não escalar a inventar uma equipa.
      --
      -- O placar vem da tabela legada e não de `game_results`: esta escalação
      -- é montada antes de os resultados serem projetados, e ler o que ainda
      -- não foi escrito devolvia sempre nulo — a equipa nunca se deduzia.
      when resultado.score_a is not null and resultado.score_b is not null
        and keeper.goals_conceded is not null
        and (resultado.score_b = keeper.goals_conceded)
          <> (resultado.score_a = keeper.goals_conceded)
        then case when resultado.score_b = keeper.goals_conceded then 'A' else 'B' end
    end as team
  ) escolhida
  where escolhida.team in ('A', 'B')
  on conflict (game_id, membership_id) do update
