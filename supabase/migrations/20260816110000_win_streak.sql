-- KickHub V2 — a sequência de vitórias, para os títulos a poderem usar.
--
-- Dos seis títulos do modelo, cinco saem de contagens que o ranking já traz:
-- golos, assistências, vitórias, craques e presenças. O sexto — três ou mais
-- vitórias seguidas — precisa da **ordem** dos jogos, que nenhuma soma guarda.
--
-- É deliberadamente sobre o presente. Conta-se do jogo mais recente para trás e
-- pára na primeira não-vitória: a marca é de estar em sequência agora, não de
-- ter estado em Março. Um empate corta tal como uma derrota, porque o que se
-- celebra são vitórias seguidas e não invencibilidade.
--
-- Os títulos em si vivem no domínio, e não aqui, porque dependem de comparar
-- todos os jogadores entre si — é a passagem que o servidor não faz.
