-- KickHub V2 — repetir o backfill volta a reconciliar a identidade.
--
-- O backfill criava os perfis com `on conflict (id) do nothing`. Fazia sentido
-- enquanto o staging só corria pseudonimizado: a segunda passagem não tinha nada
-- de novo para dizer. Deixou de fazer no momento em que o mesmo staging pode ser
-- reimportado com os nomes e as fotos verdadeiros — o `do nothing` deixava toda
-- a gente como `Jogador Browns NNN` para sempre, e a foto em `pending` sem nunca
-- lá chegar.
--
-- Agora reconcilia, mas só enquanto o perfil for uma projeção do legado. Assim
-- que alguém o reclama — `auth_user_id` deixa de ser nulo — o nome e a alcunha
-- passam a ser dessa pessoa, e nenhuma passagem do backfill os desfaz. É a mesma
-- fronteira que o claim de uso único já desenha.
