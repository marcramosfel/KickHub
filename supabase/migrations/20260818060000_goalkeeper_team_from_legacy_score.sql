-- KickHub V2 — deduzir a equipa do guarda-redes antes de o placar ser projetado.
--
-- A migration anterior deduzia a equipa a partir dos golos sofridos comparados
-- com `game_results`. Só que dentro do próprio backfill as escalações são
-- montadas antes de os resultados serem projetados: quando aquele bloco corre,
-- `game_results` ainda está vazia. A dedução devolvia sempre nulo e a linha de
-- escalação continuava a não existir — numa base nova, que é o caso do pgTAP e
-- o de qualquer restauração de raiz.
--
-- Em staging isto passou despercebido porque lá `game_results` já tinha sido
-- preenchida por uma execução anterior. O defeito só aparece na primeira vez,
-- que é precisamente quando o cutover de produção vai correr.
--
-- O placar passa a vir de `public.matches`, a tabela legada, que está restaurada
-- desde o início e não depende da ordem dos inserts. Reordenar o backfill era a
-- outra saída, mas deixava a correção dependente de uma ordem que ninguém vê ao
-- ler o bloco.
