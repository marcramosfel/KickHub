-- KickHub V2 — a equipa do guarda-redes quando o legado não a registou.
--
-- A migration anterior deu escalação a quem guardou a baliza e só existia em
-- `goalkeeper_match_stats`, mas exigia `keeper.team in ('A','B')`. Na Browns há
-- exactamente uma rodada nessas condições — e é também a única em que a coluna
-- `team` ficou nula. O filtro saltava-a, portanto a correção passava ao lado do
-- único caso que existia para corrigir: o jogador continuava com 6 defesas, um
-- jogo e um registo `0-0-0`.
--
-- O teste não apanhou isto porque o fixture que escrevi deu equipa ao guarda-
-- redes. Reproduzia a forma do problema, não o problema.
--
-- Agora a equipa vem de três sítios, por esta ordem: a coluna do próprio
-- registo, a equipa em `match_stats`, e por fim os golos sofridos comparados com
-- o placar — quem guarda a baliza de A sofre os golos de B. A inferência só se
-- aplica quando um dos lados bate e o outro não; num empate prefere-se não
-- escalar a inventar.
...
begin
  ...
  left join public.game_results resultado on resultado.game_id = keeper.match_id
  cross join lateral (
    select case
